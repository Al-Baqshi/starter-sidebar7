...
export function JobList({ jobs = mockJobs, updateJob }: JobListProps) {
  ...
  const calculateTotalCost = (job: any) => {
    const materialsCost = job.materials.reduce(
      (sum: any, material: any) =>
        sum + material.estimatedQuantity * material.unitRate,
      0
    );
    const laborCost = job.labor.reduce(
      (sum: any, labor: any) =>
        sum + labor.estimatedStaff * labor.estimatedHours * labor.hourlyRate,
      0
    );
  ...
  const handleDeleteMaterial = (job: any, materialId: string) => {
    if (job.status === "ready") return;
    const updatedMaterials = job.materials.filter(
  ...
  return (
    <div className="space-y-6">
        ...
        >
          <CardHeader>
              ...
              <div className="flex justify-between items-center">
                ...
                <div className="flex items-center space-x-4">
                  <span className="text-sm font-normal">
                    Total: ${formatNumber(job.totalCost ?? 0)}
                  </span>
                  <Select
                    value={job.status}
                    onValueChange={(value) =>
                      handleStatusChange(job, value as "draft" | "ready")
                    }
                  >
            ...
            <div className="flex justify-between items-center mt-2 text-sm text-muted-foreground">
              ...
              <span>
                {job.includedInTender ? "Included in Tender" : "Not in Tender"}
              </span>
            </div>
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xl font-bold">
                    ${formatNumber(calculateTotalCost(job))}
                  </span>
                </div>
                <div>
                    ...
                    <TableBody>
                      {job.materials.map((material: any) => (
                        <TableRow key={material.id}>
                          ...
                          <TableCell>
                            ${formatNumber(material.totalCost ?? 0)}
                          </TableCell>
                          <TableCell>
                            <Button
                              variant="ghost"
                          ...
                          <TableCell>
                            <Button
                              ...
                              size="sm"
                              onClick={() =>
                                handleDeleteMaterial(job, material.id)
                              }
                              disabled={job.status === "ready"}
                            >
                    ...
                    <TableBody>
                      {job.labor.map((labor: any) => (
                        <TableRow key={labor.id}>
                          ...
                          <TableCell>
                            ${formatNumber(labor.totalCost ?? 0)}
                          </TableCell>
                          <TableCell>
                            <Button
                              variant="ghost"
                            ...
                            >
                              <PaperclipIcon className="h-4 w-4" />
                              <span className="ml-2">
                                {labor.notes || labor.attachments?.length
                                  ? "View/Edit"
                                  : "Add"}{" "}
                                Notes & Attachments
                              </span>
                          ...
                          <TableCell>
                            <Button
                              variant="ghost"
                              size="sm"
                              onClick={() => handleDeleteLabor(job, labor.id)}
                              disabled={job.status === "ready"}
                            >
                              <Trash2 className="h-4 w-4" />
                            </Button>
      ...
      <div className="mt-8 text-right font-bold text-xl">
        Grand Total: $
        {formatNumber(
          jobs.reduce((sum, job) => sum + calculateTotalCost(job), 0)
        )}
      </div>
      <Dialog
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {dialogState.labor?.notes ||
              dialogState.labor?.attachments?.length
                ? "View/Edit"
                : "Add"}{" "}
              Notes & Attachments
            </DialogTitle>
          ...
          <div className="space-y-4">
            <Textarea
              ...
              rows={5}
            />
            {dialogState.labor?.attachments &&
              dialogState.labor.attachments.length > 0 && (
                <div>
                  <h4 className="mb-2 font-semibold">Attachments:</h4>
                  <ul className="list-disc pl-5">
                    {dialogState.labor.attachments.map(
