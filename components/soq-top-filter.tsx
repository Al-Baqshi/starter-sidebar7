...
export function SOQTopFilter({ categories, selectedJobId, onSelectJob, onAddCategory, onRemoveCategory }: SOQTopFilterProps) {
  ...
  return (
    <div className="w-full bg-background p-4 border-b">
      ...
      <ScrollArea className="h-[200px] p-2">
        <Button variant="outline" className="w-full mb-2" onClick={() => setIsAddCategoryDialogOpen(true)}>
          <Plus className="mr-2 h-4 w-4" />
          Add Category
        </Button>
        <Accordion type="multiple" className="w-full">
          {filteredCategories.map(category => (
            <AccordionItem value={category.id} key={category.id} className="relative">
